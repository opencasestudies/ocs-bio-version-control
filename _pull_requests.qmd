...
Recall that in the initial description of branches, you learned that branches can be used for development of a new part of a project, separate from the stable version of a repository. A pull request is a way to take a collection of commits on a branch and request to merge those into another branch, typically "main". A pull request will provide a list of each file that is changed in the branch, as well as the difference between each file and the version of that file on the branch that you are trying to merge your changes into. While you can review your own pull requests, they also have great built-in features for collaborators to review your pull request. 
...
- Navigate back to "https://github.com/your_username/infant_gut_analysis.git" (replacing your username). 
...
#### **Plots to compare study cohorts**

This pull request is in response to the request from **`@collab.o.rator`** to compare the original studies in the meta-analysis. Our goal is to make plots to compare the study sample sizes and time points at which each study collected samples. This pull request adds the following files to the repository:

- "03_visualize_study_cohorts.qmd": which reads in the cleaned data and makes two figures to compare the cohorts
- "figures/study_size_by_category.png": a bar chart comparing the number of samples included in each study, separated by whether a sample is from an infant or a mother
- "figures/sampling_timepoints.png": a histogram of days that samples were taken on, separated by whether a sample is from an infant or a mother and faceted by study

This pull request also updates "README.md" to document these new files.

**`@collab.o.rator`**, could you please let me know if these figures are what you were looking for? Are there any additional plots that you'd like me to include, or different characteristics of the cohorts that you would like me to visualize? I would also appreciate a review of the code that I used to generate these figures, to make sure that I'm using the correct data. 
...
It is recommended to not let your pull requests get too large. A pull request should be a set of changes that address a certain component of an analysis. Try to avoid combining multiple parts of a project in a single pull request. If you decide to start working on a different part of a project, you can create a new branch for this, and eventually submit a new pull request from that branch. This is a great way to stay organized for complex projects that have different components happening in parallel. 
