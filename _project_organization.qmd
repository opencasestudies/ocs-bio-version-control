...
::::
::::{.inner_block}
Although this is not directly related to {{< glossary "version control" >}}, there are some useful ideas to know about organizing a scientific project, that will improve {{< glossary "reproducibility" >}}. While most people have their own approaches to scientific project organization, there are a few principles that most successful organization schemes have in common. More information can be found in [this reproducibility course](https://jhudatascience.org/Reproducibility_in_Cancer_Informatics/organizing-your-project.html).

- file names are concise but informative
- there is one central folder to contain all files for each project
- there is a central file (often called the {{< glossary "README" >}}) that explains what files are included and how to use them to run the analysis
...
The best way to avoid tracking a file is to add it to your {{< glossary ".gitignore" >}} file. If a file or folder is included in your .gitignore file, it will not be staged when you use the command `git add .`. 
...
However, large data files or results that are not expected to change over time should generally not be tracked with Git at all. Instead, they are better stored in dedicated research data repositories such as [figshare](https://figshare.com) or [Zenodo](https://zenodo.org), which are designed for long-term storage, sharing, and citation. If data files are not tracked by Git or Git LFS, and are not therefore not included in the GitHub repository, it is important for the repository to contain download instructions for these data files for other users who may want to run the code and reproduce results. The README file is a good place to include these download instructions. 
