...
First, we will make sure that we are on the correct branch. We want to use a development branch for this analysis, which we will request to merge back into the main branch once the analysis is ready. We already created the branch "study_comparison" in the [**Introduction to Git and GitHub**](https://www.opencasestudies.org/ocs-bio-version-control/#branches) section, so we just need to make sure that we are working on that branch.
...
We will now use `ggsave()` to save this plot in the figures folder. **Include this code in your .qmd document** to save this bar plot under the filename "study_sizes_by_cateogory.png" within the "figures" folder.
...
Finally, we will save this plot in the figures folder. **Include this code in your .qmd document** to save this plot with the filename "sampling_timepoints.png" within the "figures" folder.
