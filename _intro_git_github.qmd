...
Git is a version control tool, and GitHub is an website for hosting files that are tracked with Git. Git works locally on your computer to record different versions of a project, and GitHub works online to provide a digital backup of files, let you connect with collaborators, and optionally create a web presence for your project. 
...
```{r, fig.alt="Commits made to a repository called analysis_project. The first commit has the message 'Create repo and add data' and data.csv is added. The second commit has the message 'Add code to clean data' and data.csv has no change and code.R is added. The third commit has the message 'Add code for initial plots' and data.csv has no change and code.R is edited. The fourth commit has the message 'Fix plot themes' and data.csv has no change and code.R is edited.", out.width = "100%", echo = FALSE}
ottrpal::include_slide("https://docs.google.com/presentation/d/1QjS4NwVgB_oZX5YCHKCzVe9x4XRWhNv-N7iP26JXHXo/edit?slide=id.g3fade600e50_0_0#slide=id.g3fade600e50_0_0")
```
...
```{r, fig.alt="Changes to code.R file across commits. In Commit 2 the file is added, and code is added (denoted with plus marks) to load the data and remove missing ids. In Commit 3 the original code is still there, and code to plot is added afterwards (again denoted with plus marks). In Commit 4, one old line of code making the plot is removed and two new lines are added.", out.width = "100%", echo = FALSE}
ottrpal::include_slide("https://docs.google.com/presentation/d/1QjS4NwVgB_oZX5YCHKCzVe9x4XRWhNv-N7iP26JXHXo/edit?slide=id.g409bc527eed_0_33#slide=id.g409bc527eed_0_33")
```

Here you can see that green colored sections are additions and red colored sections are deletions. Git will not automatically make commits for you. You need to decide when to commit. If you want to return to a previous version of a project, you can only return to the version of files at any commit.
...
Typically, it is easier to create a new repository remotely with GitHub when started a version-controlled project from scratch. However, when you have already started work on a project and decide to version-control existing resources, you will want to create your new repository locally. 

In this case study we will practice both approaches. First, in the [following section](https://www.opencasestudies.org/ocs-bio-version-control/#getting-started-with-git-and-github), you will practice creating a new repository from GitHub and cloning it to your local computer. Later, in the [section in which you create a repository for the case study analysis](https://www.opencasestudies.org/ocs-bio-version-control/#create-a-version-controlled-repository-for-your-project), you will practice working with an existing set of files and version-controlling them locally with Git. 
...
Now that you have a GitHub account, you can create your first repository. The goal of this repository is to get you set up and to practice with Git and GitHub, you will later make another repository to use for the case study analysis.
...
By default your repository will be set to public, which means that anyone can view all of the files contained in it. If you would prefer that only you and collaborators that you add can view the contents, you can set the visibility to private. Please note that this is not ***truly*** private, so please do not host protected heath information or other forms of data that require extra protection on GitHub even with a private repo.
...
To check whether your Mac has an Apple silicon chip or an Intel chip, click the Apple menu 
-> "About This Mac". If the information window shows "Chip" then your Mac uses Apple silicon. If the information window shows "Processor" then your Mac uses an Intel processor. 
...
GitHub no longer supports using passwords to connect Git and GitHub, so you'll instead need to use a Personal Access Token (PAT). To create a token, [click this link to go to your settings for tokens](https://github.com/settings/tokens). In order to connect your token to your computer do the following:
...
Next, we need to trigger a GitHub credentials check. We will do this by cloning the repository that you made (this means we will make a local version of this repo). See this [section if you still need to make the repository](https://www.opencasestudies.org/ocs-bio-version-control/#creating-a-new-github-repository). Run the following, replacing "your_username" with your username:
...
Now, we will return to the project with Collab. O. Rator. In the previous section, you created a repository on GitHub, and then cloned to make a local copy. Here, you will practice the other approach to creating a new version-controlled repository. You will start with a local folder, track it with Git, and then create a remote version of the repository on GitHub. 

First, download the .zip file [here](https://github.com/opencasestudies/version_controlled_infant_gut_analysis/blob/main/version_controlled_infant_gut_analysis.zip). Your browser may automatically unzip this file to create a folder called "infant_gut_files", if not then unzip the file manually (this typically involves right clicking or double clicking depending on your operating system). 
...
ottrpal::include_slide("https://docs.google.com/presentation/d/1QjS4NwVgB_oZX5YCHKCzVe9x4XRWhNv-N7iP26JXHXo/edit?slide=id.g3f9c55c112a_0_130#slide=id.g3f9c55c112a_0_130")
```
...
- If you are working on a repository with multiple contributors, make sure to pull each time you work in order to work on the most up-to-date version of the repository. If you don't, Git will let you know and help you to do so (check out this resource about more complicated [merging when you forgot to pull](https://medium.com/@Spritan/merge-vs-rebase-vs-fast-forward-mastering-git-integration-strategies-3ba8e652d93e)).  
