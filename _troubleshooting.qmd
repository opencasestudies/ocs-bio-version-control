...
ottrpal::include_slide("https://docs.google.com/presentation/d/1TEi1D35oiiC21mYR0cKjWuOOpuxm3fTGyCXSqFmjL3I/edit?slide=id.g3f139628ce6_0_344#slide=id.g3f139628ce6_0_344")
```
...
Including the `git commit --ammend` rewrites the most recent commit as if the large file was not added. Don't close the terminal yet, we will come back to it. 
...
Return to the terminal. Repeat the `git commit --ammend` command, which will add this update to the .gitignore to the most recent commit. 
...
Use the following command, replacing "path_to_file" with the path to the file to stop tracking, including the file name. Including the `git commit --ammend` rewrites the most recent commit as if the large file was not added.
...
Then, return to the command line and stage and stage and commit these changes. Note that we include `git commit --ammend` a second time to add the changes to the .gitignore file to the previous commit. 
